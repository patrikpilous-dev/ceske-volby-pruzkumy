...
Záložní zdroj: ct24.cz (zpravodajské články)
"""
...
def fetch(url):
    try:
        ...
        print(f"[ERROR] Kantar fetch {url}: {e}")
        return None
...
def get_kantar_links(max_pages=5):
    ...
    for page in range(1, max_pages + 1):
        ...
        for a in soup.find_all("a", href=True):
            ...
            if any(kw in txt for kw in ["volební", "model", "preference", "volby", "strany"]):
...
def get_ct24_links(max_pages=5):
    ...
    for page in range(1, max_pages + 1):
        ...
        for a in soup.find_all("a", href=True):
            href = a["href"]
            txt  = a.get_text().lower()
            if "kantar" in txt and any(kw in txt for kw in ["volební", "model", "preference"]):
...
def scrape_article(url, title="", source="kantar"):
    soup = fetch(url)
    if not soup:
        return None
    print(f"[Kantar] {url}")
    ...
    if not any(kw in text.lower() for kw in ["volební model", "preference", "kantar", "česká televize"]):
        return None
    ...
    poll_type = "preference" if "preference" in text.lower() and "volební model" not in text.lower() else "model"
...
def run_scraper(historical=False):
    ...
    # Zkus primárně kantar.com, pak CT24 jako zálohu
